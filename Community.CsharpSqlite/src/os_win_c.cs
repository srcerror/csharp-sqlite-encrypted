...
static int winLogErrorAtLine(
  ...
  sqlite3_log(errcode, "os_win.c:{0}: ({1}) {2}({3}) - {4}", iLine, iErrno, zFunc, zPath, zMsg);
      ...
      )
      {
        ...
        while ( cnt-- > 0 && res == 0 )//(res = LockFile(pFile.fs.SafeFileHandle.DangerousGetHandle().ToInt32(), PENDING_BYTE, 0, 1, 0)) == 0)
        {
          ...
          catch ( Exception e )
          {
            ...
            */
#if SQLITE_DEBUG
            OSTrace( "could not get a PENDING lock. cnt={0}\n", cnt );
#endif
#if SQLITE_WINRT
...
error = 1;
      ...
      */
      ...
      {
#if SQLITE_DEBUG
        OSTrace( "LOCK FAILED {0} trying for {1} but got {2}\n", pFile.fs.GetHashCode(),
    ...
    */
    static int winCheckReservedLock( sqlite3_file id, ref int pResOut )
    {
...
#if SQLITE_DEBUG
#if WINDOWS_PHONE || SQLITE_SILVERLIGHT
        OSTrace("TEST WR-LOCK {0} {1} (local)\n", pFile.fs.GetHashCode(), rc);
#else
        OSTrace( "TEST WR-LOCK {0} {1} (local)\n", pFile.fs.Name, rc );
#endif
#endif
      ...
      else
      {
        ...
        rc = 1 - rc; // !rc
#if SQLITE_DEBUG
        OSTrace("TEST WR-LOCK {0} {1} (remote)\n", pFile.fs.GetHashCode(), rc);
#endif
      }
      pResOut = rc;
      return SQLITE_OK;
  ...
  while( (p = *pp)!=0 ){
    if( p->nRef==0 ){
      ...
      for(i=0; i<p->nRegion; i++){
        ...
        OSTrace(("SHM-PURGE pid-{0} unmap region={1} {2}\n",
                 (int)GetCurrentProcessId(), i,
                 bRc ? "ok" : "failed"));
        bRc = CloseHandle(p->aRegion[i].hMap);
        OSTrace(("SHM-PURGE pid-{0} close region={1} {2}\n",
                 (int)GetCurrentProcessId(), i,
                 bRc ? "ok" : "failed"));
      }
...
static int winShmLock(
  ...
  sqlite3_mutex_leave(pShmNode->mutex);
  OSTrace(("SHM-LOCK shmid-{0}, pid-{1} got {2:x),{3:x) {4}\n",
  ...
  if( pShmNode->nRegion<=iRegion ){
    ...
    */
    ...
    while( pShmNode->nRegion<=iRegion ){
      ...
      OSTrace(("SHM-MAP pid-%d create region={0} nbyte={1} {2}\n",
      ...
      if( hMap ){
        ...
        OSTrace(("SHM-MAP pid-%d map region={0} offset={1} size={2} {3}\n",
