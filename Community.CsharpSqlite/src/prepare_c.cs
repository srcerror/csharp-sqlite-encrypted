      ...
      {
        {
          ...
          pData.pzErrMsg = string.Format("malformed database schema ({0})", zObj);

          if (!String.IsNullOrEmpty(zExtra))
          {
            //sqlite3MAppendf( db, pData.pzErrMsg, "%s - %s", pData.pzErrMsg, zExtra );
            pData.pzErrMsg = string.Format("{0} - {1}", pData.pzErrMsg, zExtra);
          }
        }
...
//#define temp_master_schema 0
      ...
      if (!sqlite3BtreeIsInReadTrans(pDb.pBt))
      {
        ...
        if (rc != SQLITE_OK)
        {
          ...
          if (pDb.pBt.pBt.pSchema.file_format == 2)
          {
            ...
            pzErrMsg = string.Format("{0} (wal format detected)", sqlite3ErrStr(rc));
          }
          else
          {
            //sqlite3SetString(ref pzErrMsg, db, "%s", sqlite3ErrStr(rc));
            pzErrMsg = string.Format("{0}", sqlite3ErrStr(rc));
          }
#else
            //sqlite3SetString( ref pzErrMsg, db, "%s", sqlite3ErrStr( rc ) );
            pzErrMsg = string.Format("{0}", sqlite3ErrStr(rc));
      ...
      {  /* text encoding */
        ...
        else
        {
          ...
          if (meta[BTREE_TEXT_ENCODING - 1] != ENC(db))
          {
            ...
            pzErrMsg = "attached databases must use the same text encoding as main database";
            rc = SQLITE_ERROR;
            goto initone_error_out;
          }
        }
      ...
      */
      ...
      if (pDb.pSchema.file_format > SQLITE_MAX_FILE_FORMAT)
      {
        ...
        pzErrMsg = "unsupported file format";
