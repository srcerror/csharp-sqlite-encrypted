        ...
        */
        ...
        for ( j = cnt = 0; j < i; j++ )
        {
          if ( aCol[j].zName.Equals( zName, StringComparison.OrdinalIgnoreCase ) )
          {
            ...
            zNewName = string.Format("{0}:{1}", zName.Substring( 0, nName ), ++cnt );
            sqlite3DbFree( db, ref zName );
            zName = zNewName;
            j = -1;
            if ( zName == "" )
    ...
    */
    ...
    )
    {
      if ( pParse.explain == 2 )
      {
        ...
        string zEqp = string.Format("SCAN TABLE {0} {1}{2}(~{3} rows)",
            pTab.zName,
...
*/
    void sqlite3PrintExpr( Expr p )
    {
      ...
      {
        sqlite3DebugPrintf( "({0}", p.u.zToken );
      }
      else
      {
        sqlite3DebugPrintf("({0}", p.op);
      }
      if ( p.pLeft != null )
      {
        sqlite3DebugPrintf( " " );
    ...
    {
      var indstring = new string(' ', indent);
      sqlite3DebugPrintf("{0}SELECT({1}) ", indstring, p);
      ...
      if ( p.pSrc != null )
      {
        ...
        for ( i = 0; i < p.pSrc.nSrc; i++ )
        {
          ...
          sqlite3DebugPrintf( "{0}{1} ", indstring, zPrefix);
          zPrefix = "";
          if ( pItem.pSelect != null )
          {
            sqlite3DebugPrintf( "(\n" );
            sqlite3PrintSelect( pItem.pSelect, indent + 10 );
            sqlite3DebugPrintf( "{0}{1})", indstring, new string(' ', 8));
          }
          else if ( pItem.zName != null )
          {
            sqlite3DebugPrintf( "{0}", pItem.zName );
          }
          if ( pItem.pTab != null )
          {
            sqlite3DebugPrintf( "(vtable: {0})", pItem.pTab.zName );
          }
          if ( pItem.zAlias != null )
          {
            sqlite3DebugPrintf( " AS {0}", pItem.zAlias );
          }
          if ( i < p.pSrc.nSrc - 1 )
          {
            sqlite3DebugPrintf( "," );
      ...
      if ( p.pWhere != null )
      {
        sqlite3DebugPrintf( "{0}WHERE ", indstring);
        sqlite3PrintExpr( p.pWhere );
        sqlite3DebugPrintf( "\n" );
      }
      if ( p.pGroupBy != null )
      {
        sqlite3DebugPrintf("{0} GROUP BY ", indstring);
        sqlite3PrintExprList( p.pGroupBy );
        sqlite3DebugPrintf( "\n" );
      }
      if ( p.pHaving != null )
      {
        sqlite3DebugPrintf("{0} HAVING ", indstring);
        sqlite3PrintExpr( p.pHaving );
        sqlite3DebugPrintf( "\n" );
      }
      if ( p.pOrderBy != null )
      {
        sqlite3DebugPrintf("{0} ORDER BY ", indstring);
